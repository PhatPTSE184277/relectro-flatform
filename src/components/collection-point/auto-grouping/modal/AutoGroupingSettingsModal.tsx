...
const AutoGroupingSettingsModal: React.FC<Props> = ({ open, onClose, onConfirm, value, onChange, loading = false }) => {
  if (!open) return null;
        ...
        <div className='flex-1 overflow-y-auto p-6 space-y-4 bg-white'>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            ...
            <div className='bg-gray-50 rounded-xl border border-gray-100 p-4'>
              ...
              <div className='text-xs text-gray-500 mb-2'>HH:mm</div>
            ...
            <div className='bg-gray-50 rounded-xl border border-gray-100 p-4 md:col-span-2'>
              ...
              <div className='text-xs text-gray-500 mb-2'>Mức tải để kích hoạt ghép xe</div>
              <CustomNumberInput value={value.loadThresholdPercent} onChange={(n) => onChange({ ...value, loadThresholdPercent: n })} min={0} max={100} className='w-full px-3 py-2 border border-primary-200 rounded-lg text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent' />
            </div>
          </div>
        </div>

        <div className='flex justify-end items-center gap-3 p-5 border-t border-primary-100 bg-white'>
          <button onClick={handleConfirm} disabled={loading} className={`px-4 py-2 rounded-lg font-medium transition ${loading ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-primary-600 text-white hover:bg-primary-700 cursor-pointer'}`}>
