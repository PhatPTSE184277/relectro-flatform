...
const EditSystemConfigModal: React.FC<EditSystemConfigModalProps> = ({
    ...
    const handleConfirm = () => {
        if (config) {
            onConfirm(config.systemConfigId, value);
        }
        onClose();
    };
            ...
            <div className='relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden z-10 max-h-[90vh] animate-fadeIn'>
                ...
                <div className='flex-1 overflow-y-auto p-6 space-y-6 bg-white'>
                    <SummaryCard
                        items={[
                            {
                                icon: <KeyRound size={18} className='text-primary-400' />,
                                label: 'Key',
                                value: config.key,
                            },
                            {
                                icon: <Tag size={18} className='text-primary-400' />,
                                label: 'Tên hiển thị',
                                value: config.displayName,
                            },
                            {
                                icon: <Layers size={18} className='text-primary-400' />,
                                label: 'Nhóm',
