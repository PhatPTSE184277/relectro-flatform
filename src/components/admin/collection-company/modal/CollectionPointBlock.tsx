...
interface CollectionPointBlockProps {
    open: boolean;
    onClose: () => void;
    onConfirm: () => void;
    loading?: boolean;
...
const CollectionPointBlock: React.FC<CollectionPointBlockProps> = ({ open, onClose, onConfirm, loading }) => {
    if (!open) return null;
            ...
            <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden z-10 animate-fadeIn">
                ...
                <div className="p-6 bg-gray-50">
                    <p className="text-gray-700 text-base">
                        Bạn có chắc chắn muốn <span className="font-semibold text-red-600">khóa</span> điểm thu gom này không?
                    </p>
                </div>
                ...
                <div className="flex justify-end gap-3 p-5 border-t border-gray-100 bg-gray-50">
                    <button
                        onClick={onConfirm}
                        disabled={loading}
                        className="px-5 py-2 rounded-lg font-medium text-white cursor-pointer shadow-md transition-all duration-200 bg-red-600 hover:bg-red-700 flex items-center gap-2 disabled:opacity-40"
                    >
...
export default CollectionPointBlock;
