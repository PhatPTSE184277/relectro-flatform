...
import { getProductsBySmallPoint } from '@/services/collection-point/ProductQueryService';

interface ProductQueryContextType {
  loading: boolean;
  products: any[];
  error: string | null;
  fetchProducts: (smallPointId: number, workDate: string) => Promise<void>;
  clearProducts: () => void;
}
...
export const ProductQueryProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchProducts = useCallback(async (smallPointId: number, workDate: string) => {
    setLoading(true);
    setError(null);
    try {
      const data = await getProductsBySmallPoint(smallPointId, workDate);
