...
import { SystemConfig, AutoAssignSettings, AutoGroupingSettings, WarehouseLoadThresholdConfig } from '@/services/admin/SystemConfigService';
import AutoAssignSettingsModal from '@/components/admin/system-config/modal/AutoAssignSettingsModal';
import AutoGroupingSettingsModal from '../../../components/admin/system-config/modal/AutoGroupingSettingsModal';
...
const SystemConfigPage: React.FC = () => {
    ...
    const [showAutoGroupingModal, setShowAutoGroupingModal] = useState(false);
    ...
    const [autoGroupingDraft, setAutoGroupingDraft] = useState<AutoGroupingSettings>({
        collectionUnitId: '',
        isEnabled: false,
        scheduleTime: '00:00',
    ...
    const groupNames = [
        ...
        { value: 'AUTO_GROUPING_SETTINGS', label: 'Tự động phân xe' },
    ...
    // Load settings whenever the tab is selected.
    useEffect(() => {
        ...
        if (groupName === 'AUTO_GROUPING_SETTINGS') {
            const selected = configs.find((cfg) => String(cfg.scpName || '') === selectedScpName) || configs[0];
            const scpId = String(
                selected?.smallCollectionPointId ||
                selected?.scpId ||
                selected?.collectionUnitId ||
                selected?.systemConfigId ||
                ''
            );

            if (scpId) {
                void fetchAutoGroupingSettings(scpId);
            }
        }
    }, [groupName, fetchAutoAssignSettings, fetchAutoGroupingSettings, selectedScpName, configs]);
    ...
    const openAutoGroupingModal = () => {
        const current = autoGroupingSettings;
        setAutoGroupingDraft({
            collectionUnitId: String(current?.collectionUnitId ?? selectedScpName ?? ''),
            isEnabled: Boolean(current?.isEnabled ?? false),
            scheduleTime: String(current?.scheduleTime ?? '00:00'),
            loadThresholdPercent: Number(current?.loadThresholdPercent ?? 0)
        });
        setShowAutoGroupingModal(true);
    };
        ...
        <div className='max-w-full mx-auto px-4 sm:px-6 lg:px-8 pt-8'>
            ...
            {groupName === 'AUTO_ASSIGN_SETTINGS' ? (
                <div className='bg-white rounded-2xl shadow-lg border border-gray-100 mb-6'>
                    ...
                    <div className='overflow-x-auto'>
                        <div className='max-h-105 overflow-y-auto'>
                            <table className='min-w-full text-sm text-gray-800 table-fixed'>
                                ...
                                <tbody>
                                    {autoAssignLoading && !autoAssignSettings ? Array.from({ length: 4 }).map((_, idx) => (
                                        <tr key={idx} className='border-b border-gray-100 hover:bg-primary-50/40'>
                                            ...
                                            <td className='py-3 px-4 text-center'><div className='inline-block h-4 bg-gray-200 rounded w-32 animate-pulse' /></td>
                                        </tr>
                                    )) : autoAssignRows.map((row, idx, arr) => (
                                        <tr key={row.key} className={`${idx !== arr.length - 1 ? 'border-b border-primary-100' : ''} ${idx % 2 === 0 ? 'bg-white' : 'bg-primary-50'}`}>
                    ...
                    <AutoAssignSettingsModal open={showAutoAssignModal} onClose={() => setShowAutoAssignModal(false)} onConfirm={handleSaveAutoAssign} value={autoAssignDraft} onChange={setAutoAssignDraft} loading={autoAssignLoading} />
                </div>
            ) : groupName === 'AUTO_GROUPING_SETTINGS' ? (
                <div className='bg-white rounded-2xl shadow-lg border border-gray-100 mb-6'>
                    <div className='flex items-center justify-between gap-2 px-6 py-4 border-b border-gray-100 bg-primary-50'>
                        <div>
                            <h2 className='text-lg font-bold text-gray-900'>Tự động phân xe</h2>
                            <p className='text-sm text-gray-500'>Cấu hình ghép xe tự động theo đơn vị thu gom.</p>
                        </div>
                        <div>
                            <button onClick={openAutoGroupingModal} disabled={autoGroupingLoading} className={`px-4 py-2 rounded-lg font-medium transition flex items-center gap-2 ${!autoGroupingLoading ? 'bg-primary-600 text-white hover:bg-primary-700 cursor-pointer' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}>
                                <Edit size={16} />
                                Chỉnh sửa
                            </button>
                        </div>
                    ...
                    <div className='overflow-x-auto'>
                        <table className='min-w-full text-sm text-gray-800 table-fixed'>
                            ...
                            <tbody>
                                <tr className='border-b border-gray-100'><td className='py-3 px-4 text-gray-700'>Trạng thái</td><td className='py-3 px-4 text-center font-medium text-gray-900'>{autoGroupingSettings?.isEnabled ? 'Bật' : 'Tắt'}</td></tr>
                                <tr className='border-b border-gray-100 bg-primary-50'><td className='py-3 px-4 text-gray-700'>Giờ chạy</td><td className='py-3 px-4 text-center font-medium text-gray-900'>{autoGroupingSettings?.scheduleTime || '-'}</td></tr>
                                <tr><td className='py-3 px-4 text-gray-700'>Ngưỡng tải (%)</td><td className='py-3 px-4 text-center font-medium text-gray-900'>{String(autoGroupingSettings?.loadThresholdPercent ?? '-')}</td></tr>
                            </tbody>
                        </table>
                    </div>
                    <AutoGroupingSettingsModal open={showAutoGroupingModal} onClose={() => setShowAutoGroupingModal(false)} onConfirm={async (payload) => { await saveAutoGroupingSettings(payload); }} value={autoGroupingDraft} onChange={setAutoGroupingDraft} loading={autoGroupingLoading} />
                </div>
            ) : groupName === 'LoadThreshold' ? (
                <>
                    <LoadThresholdList
